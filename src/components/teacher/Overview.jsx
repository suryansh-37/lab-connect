...
import { Zap, TrendingUp, MoreVertical, Users, ClipboardList, Lightbulb } from 'lucide-react';
...
const Overview = ({ setActiveSection, onOpenRoom }) => (
  <motion.div variants={containerVariants} initial="hidden" animate="show">
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '2.5rem' }}>
      ...
      <div style={{ background: '#fef3c7', color: '#854d0e', padding: '0.75rem 1.5rem', borderRadius: '25px', fontWeight: 700, display: 'flex', alignItems: 'center', gap: '0.5rem' }}><Zap size={18}/> Next Class: Biology 101 @ 10:30 AM</div>
    </div>
        ...
        <motion.div variants={containerVariants}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }}>
            {teachingLabs.map((lab) => (
              <PopCard key={lab.id} style={{ padding: '0', overflow: 'hidden' }} onClick={() => onOpenRoom && onOpenRoom('stream', lab.title)}>
                <div style={{ height: '140px', position: 'relative' }}><img src={lab.banner} alt={lab.title} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /><div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.3)' }} /><div style={{ position: 'absolute', top: '1rem', left: '1rem', background: 'rgba(255,255,255,0.2)', padding: '0.4rem 1rem', borderRadius: '15px', fontWeight: 700, fontSize: '0.75rem', color: 'white' }}>Session {lab.session}</div></div>
                <div style={{ padding: '1.5rem' }}><h3 style={{ fontSize: '1.1rem', fontWeight: 800, marginBottom: '0.3rem' }}>{lab.title}</h3><p style={{ color: 'var(--text-muted)', fontSize: '0.8rem', marginBottom: '1.2rem' }}>{lab.code}</p><div style={{ height: '6px', background: 'var(--bg-main)', borderRadius: '3px', width: '100%' }}><div style={{ height: '100%', width: `${lab.progress}%`, background: '#10b981' }} /></div></div>
              </PopCard>
            ))}
          </div>
        </motion.div>
