    ...
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '1rem 2.5rem', background: 'var(--bg-card)', borderBottom: '1px solid var(--border)', position: 'sticky', top: 0, zIndex: 10 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem' }}>
        <div style={{ position: 'relative', width: '250px' }}>
          ...
          <input type="text" placeholder="Search students, resources..." style={{ width: '100%', padding: '0.5rem 1rem 0.5rem 2.5rem', borderRadius: '20px', border: '1px solid var(--border)', background: 'var(--bg-main)', outline: 'none', fontSize: '0.85rem' }} />
        </div>
        <Bell size={20} style={{ color: 'var(--text-muted)', cursor: 'pointer' }} />
        <Settings size={20} style={{ color: 'var(--text-muted)', cursor: 'pointer' }} />
        <div style={{ width: '35px', height: '35px', borderRadius: '50%', background: '#0284c7', color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 'bold' }}>T</div>
      </div>
    </div>
  );
};
