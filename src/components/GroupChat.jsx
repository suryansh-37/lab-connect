...
const GroupChat = ({ subject, userName, userRole, onBack, isEmbedded }) => {
  ...
  const fetchMessages = async () => {
      try {
          ...
          const res = await fetch(`${API_BASE_URL}/api/messages/${encodeURIComponent(activeRoomId)}?${q}`);
          if (res.ok) {
  ...
  const checkSessionAlive = async () => {
      ...
      try {
          const res = await fetch(`${API_BASE_URL}/api/sessions/status/${encodeURIComponent(activeRoomId)}`);
          if (res.ok) {
              const data = await res.json();
              if (!data.active) {
  ...
  const handleTerminateSession = async () => {
      try {
          await fetch(`${API_BASE_URL}/api/sessions/by-title/${encodeURIComponent(activeRoomId)}`, { method: 'DELETE' });
